"""
scripts/phase8_visualize_showcase.py
=======================================
Génère, pour la présentation Phase 8, une figure par catégorie de résultat
du pipeline MODÈLE APPRIS complet (étage 1 = `DepthmapPoseRegressor` avec
profil de corrélation + tête miroir dédiée, étage 2 = `trimmed_icp_normals`),
en condition réelle (masque CNN `thresh0.3`) — la MÊME chaîne que
`scripts/phase8_pipeline_learned_check.py --strategy thresh03` (aucune
réimplémentation de la logique de matching/raffinement, `run_learned_stage1`
est importée telle quelle).

Quatre catégories cherchées, une paire par catégorie (mêmes seuils et même
logique de groupe que `phase8_pipeline_learned_check.py`, section 9A) :
  - not_eligible      : étage 1 échoue (pas assez de points, ou
                        `build_correspondences_nn` ne trouve rien).
  - fail              : pose finale (après ICP) rate Pose@30°/0.1  (groupe C)
  - pose30_not_strict : pose finale sous 30°/0.1 mais pas sous le seuil
                        strict (5°/0.02)                            (groupe B)
  - strict_success    : pose finale sous le seuil strict            (groupe A)

Usage (sur le serveur) :
    CUDA_VISIBLE_DEVICES=1 python scripts/phase8_visualize_showcase.py \\
        --regressor_ckpt output/phase8_depthmap_regressor_thresh03_mirror_head/best.ckpt \\
        ...
        --out_dir /tmp/student7/phase8_showcase_learned
"""
...
def make_figure(category, obj_idx, raw_i, raw_j, frac_i_base, frac_j_base,
    ...
    if stage1_res["reached_stage"] == "pose_computed":
        ...
        ax6.set_title("Étage 1 (modèle appris, pose grossière)\n"
    ...
    if stage1_res["reached_stage"] == "pose_computed":
        lines += [
            f"confiance miroir : {stage1_res['mirror_prob']:.3f}",
            f"n correspondances : {stage1_res['n_corr']}",
    ...
    fig.suptitle(f"Modèle appris (profil + tête miroir) — {CATEGORY_TITLES[category]}", fontsize=13)
...
def main():
    ...
    parser.add_argument("--ckpt", required=True, help="Checkpoint CNN (masque thresh0.3)")
    ...
    with torch.no_grad():
        for idx, batch in enumerate(cnn_loader):
            ...
            stage1_res = run_learned_stage1(regressor, frac_i_zoom, frac_j_zoom,
                                             nrm_i_zoom, nrm_j_zoom, R_ij_gt, t_ij_gt, device)
